...
//各class的属性列表会缓存在同一个map上，所以需要区分是否支持跨线程使用，默认支持。
#ifndef GYD_JSON_OBJECT_ATOMIC
#   define GYD_JSON_OBJECT_ATOMIC 1
#endif

@implementation NSObject (GYDJSONObject)

#if GYD_JSON_OBJECT_ATOMIC == 1
static NSRecursiveLock *_PropertyMapCacheLock = nil;
//现在改成类别，就不要用initialize方法了
+ (void)load {
...
+ (NSArray<GYDJSONPropertyInfo *> *)gyd_propertyInfoArrayWithoutCache {
    ...
    gyd_class_enumerateMethod(self, YES, ^(const char * _Nonnull methodName, BOOL * _Nonnull stop) {
        ...
        gyd_class_getPropertyType_block(property_getAttributes(pro), ^(char * _Nullable className, char * _Nullable type) {
            if (NULL != className) {
                info->_propertyClass = NSClassFromString([NSString stringWithCString:className encoding:NSUTF8StringEncoding]);
            }
            if (NULL != type) {
                info->_encodeType = [NSString stringWithCString:type encoding:NSUTF8StringEncoding];
            }
        });
